...
void RenderScene()
{
	...
	// 셰이더에 텍스쳐를 매핑해주는데 _Tex 라는 접미사가 붙는다.
	// .fx 파일을 열어서 직접 확인 한 다음에 적어보자
	gpTextureMappingShader->SetTexture("DiffuseMap_Tex", gpEarthDM);

	// 이제 셰이더가 가진 값들을 그릴 물체들에 적용하라고 GPU에게 명령한다.
	// 모양과 함수들을 잘 보자
	UINT numPasses = 0;
	gpTextureMappingShader->Begin(&numPasses, NULL); // Begin()과 End() 사이에
	{
		for (UINT i = 0; i < numPasses; ++i)
		{
			gpTextureMappingShader->BeginPass(i); // BeginPass()와 EndPass() 가 있고
			{
				// 구체를 그린다.
