...
// 프로그램 이름
const char* gAppName = "UVAnimation 쉐이더 프레임워크";
// 회전값
...
void RenderScene()
{
	...
	gpUVAnimationShader->SetFloat("gWaveHeight", 4);
	...
	// 모양과 함수들을 잘 보자
	UINT numPasses = 0;
	gpUVAnimationShader->Begin(&numPasses, NULL); // Begin()과 End() 사이에
	{
		for (UINT i = 0; i < numPasses; ++i)
		{
			gpUVAnimationShader->BeginPass(i); // BeginPass()와 EndPass() 가 있고
			{
				// 구체를 그린다.
