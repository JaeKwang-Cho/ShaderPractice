...
// 프로그램 이름
const char* gAppName = "Normal Mapping 쉐이더 프레임워크";
// 회전값
...
void RenderScene()
{
	...
	// 모양과 함수들을 잘 보자
	UINT numPasses = 0;
	gpNormalMappingShader->Begin(&numPasses, NULL); // Begin()과 End() 사이에
	{
		for (UINT i = 0; i < numPasses; ++i)
		{
			gpNormalMappingShader->BeginPass(i); // BeginPass()와 EndPass() 가 있고
			{
				// 구체를 그린다.
